...
data_dir = out_dir = 'ape_src'
test_data = "ape3.jpg"  # for example 'roy_example.png'
CH = test_data[:-4]
use_gt_bb = False
...
for eachFile in filename:
    with open(os.path.join("./" + data_dir, CH + eachFile), 'w+') as temp_file:
        temp_file.write("")

if(not os.path.exists(os.path.join(data_dir, CH + '.pts'))):

    from models.face_of_art.menpo_functions import *
    from models.face_of_art.deep_heatmaps_model_fusion_net import DeepHeatmapsModel
    ...
    bb_dictionary = load_bb_dictionary(
        bb_dir, mode='TEST', test_data=test_data)
    bb_type = 'init'

    img_list = load_menpo_imagepython_list(
...
np.savetxt(os.path.join(
    data_dir, '{}_face_open_mouth.txt'.format(CH)), pts, fmt='%.4f')
